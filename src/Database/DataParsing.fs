...
let getEquivalentCharacters (path: string) =
    ...
    for line in File.ReadAllLines(path) do
        let m = Regex.Match(line, @"^(?<in1>\w{4})(?:\.\.(?<in2>\w{4}))? +; +(?<out>\w{4,6}) +# +(?:\[(?<n>\d)\])? +(?<description>.+)")
        if m.Success then
...
let parseRadkFile (path: string) =
    ...
    |> List.map (fun m ->
        let radical = char (m.Groups[1].Value)
        {
            Radical =
                radical
                |> replacements.TryFind
                |> Option.defaultValue radical
                |> rune
...
let getRadkEntries () =
    ...
    |> List.map (fun (radical, pair) ->
        ...
        | _ ->
            failwithf "Expected exactly one entry for %A in each radk file. Received %A." radical pair
    )
